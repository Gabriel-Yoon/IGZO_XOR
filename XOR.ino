    ...
    if (Serial.available() > 0)
    {
        ...
        for (int i = 0; i < epoch; i++)
        {
            ...
            */
            ...
            // backpropagate되는 양을 의미함. ADC(0~1023 10bit)의 범위를 맞추기 위해서 amplification_factor를 곱해준다.
            double Output_b1 = amplification_factor * error * (outputLayer.activationValue[0]) * (1 - outputLayer.activationValue[0]);

            // 앞서 정의된 Output_b1의 값에서 소자의 값을 반영한 특정값을 곱해준 만큼 역전(backpropagation)을 시켜줌(특정값은 feedforward 과정에서 곱해준 일정한 상수와 일치를 시켜주어야 한다)
            Y[0] = 0;
            Y[1] = 0;
            Y[2] = abs(Output_b1);
            Y[3] = 0;
            ...
            b3 = (32 * (double(core._ADCvalue[3]) - double(Output_b1_plus) * double((core._mid[2][3])))) / (double(core._standard[3][max]) - double(core._standard[3][min])); // 이 값은 -Output_b1 ~ Output_b1의 값을 가지며, amplification factor가 곱해진 P 펄스 발생 확률과 연관이 됨
            b4 = (32 * (double(core._ADCvalue[4]) - double(Output_b1_plus) * double((core._mid[2][4])))) / (double(core._standard[4][max]) - double(core._standard[4][min])); // 이 값은 -Output_b1 ~ Output_b1의 값을 가지며, amplification factor가 곱해진 P 펄스 발생 확률과 연관이 됨
            p[2] = error * (Activation_output_o1) * (1 - Activation_output_o1);                                                                                               // 사실 p2는 backpropagate전에 이미 결정된 값이지만, p0와 p1과 함께 표기하기 위해 이 곳에 표기, 이 때 p0, p1은 case에 따라서 부호를 달리해야할 것으로 보임

            if (error >= 0)
            {
                p[0] = double(b3 * Activation_output_h1) * (1 - Activation_output_h1) / double(amplification_factor);
                p[1] = double(b4 * Activation_output_h2) * (1 - Activation_output_h2) / double(amplification_factor);
            }
            else
            {
                p[0] = -1 * double(b3 * Activation_output_h1) * (1 - Activation_output_h1) / double(amplification_factor);
                p[1] = -1 * double(b4 * Activation_output_h2) * (1 - Activation_output_h2) / double(amplification_factor);
            }
            ...
            for (int i = 0; i < Bit_length; i++)
            {
                N1[0][i] = ((rand() % 100000) + 1 <= Q[0]) ? 1 : 0;
                N1[1][i] = ((rand() % 100000) + 1 <= Q[1]) ? 1 : 0;
                N1[2][i] = ((rand() % 100000) + 1 <= Q[2]) ? 1 : 0;
                N1[3][i] = ((rand() % 100000) + 1 <= Q[3]) ? 1 : 0;
                N1[4][i] = ((rand() % 100000) + 1 <= Q[4]) ? 1 : 0;
            ...
            for (int i = 0; i < Bit_length; i++)
            {
                N3[0][i] = ((rand() % 100000) + 1 <= Q[0]) ? 1 : 0;
                N3[1][i] = ((rand() % 100000) + 1 <= Q[1]) ? 1 : 0;
                N3[2][i] = ((rand() % 100000) + 1 <= Q[2]) ? 1 : 0;
                N3[3][i] = ((rand() % 100000) + 1 <= Q[3]) ? 1 : 0;
                N3[4][i] = ((rand() % 100000) + 1 <= Q[4]) ? 1 : 0;
